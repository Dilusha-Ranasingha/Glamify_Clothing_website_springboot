...
const AdminPageMen = () => {
  ...
  useEffect(() => {
    const fetchCards = async () => {
      try {
        const response = await axios.get("http://localhost:8080/api/mencards/getall");
    ...
    try {
      ...
      if (isUpdating) {
        ...
        const updateResponse = await axios.put(
          `http://localhost:8080/api/mencards/update/${cardData.id}`,
          formData,
          {
        ...
        // Add new card
        const addResponse = await axios.post("http://localhost:8080/api/mencards/add", formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        const newCard = {
          ...addResponse.data,
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmation) {
      try {
        await axios.delete(`http://localhost:8080/api/mencards/delete/${id}`);
